...
# When user puts only 1 or 2 lines, all 3 lists should be displayed still
...
# Allow to specify chance of a particular symbol #A: Done
...
# A : Can we modularise counting previous win totals and credit won into their own separate functions This would
# allow for easier debugging and readability BUG I found a bug, could still bet under 0 credits. This occurred
# because although it was checking enough credits, it was not checking for the * lines
...
def chooseLines():
    ...
    try:
        ...
        if lines < 1 or lines > 3:
            raise ValueError  # A: prevents the user from entering a number outside the range
    except ValueError:
...
def makeBet():
    ...
    if playerData.freeGames > 0:  # A: This should fix being able to change bet amount when you have free games
        print(
            "You have free games remaining. You must play them before changing your bet.")  # Oh, no this broke the
        # game, it makes it, so they can have a bet amount of null lmao
...
# A: I think in order to handle this it needs to be split into 3 lists, not a list of lists.
...
def featureFunc():
    ...
    while featureCounter > 0:
        xPositions = []
        for a in range(
                len(lines)):  # This line generates an int from 0-9 (length of lines) stored in 'a', it also loops
            # for the length of this int
            if random.random() < 0.5 and lines[a] != "$":  # If number in the 'a' list are not $
                for b in range(len(lines[
                                       a])):  # This line generates an int from 0-'a' stored in 'b'. Iterates over
                    # the characters in the 'a' list
                    if random.random() < .5 and lines[a][b] != "$":
                        lines[a][b] = "$"
                        xPositions.append((a, b))  # I should think of an as rows, b as columns
...
def leverPull():
    ...
    print(
        f"| Bet Amount: {playerData.betAmount} | Lines: {playerData.lines} | Free Games: {playerData.freeGames}".ljust(
            45) + "|")
    print(" " + "‾" * 44)

    # Add previousWin to previousWinData list for win tracking Instead of storing previous win here, I store what the
    # multiplier would have been. IE previousWin = 180/ 10 / 3 = 6 per 1 credit bet
